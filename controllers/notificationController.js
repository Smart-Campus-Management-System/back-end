...
exports.addNotification = async (req, res) => {
    try {
        ...
        const newUser = await Notification.create({
            message
        });
        res.status(201).json({
            success: true,
        ...
        return res.status(500).json({ error: "An error occurred while creating notifications." });
    }
}

exports.getAll = async (req, res) => {
    try {
        const allData = await Notification.find();
        res.status(200).json({
            success:true,
