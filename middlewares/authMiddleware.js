...
exports.isAdminOrTutor = async (req, res, next) => {
    try {
        if (req.user.accountType !== "Admin" && req.user.accountType !== "Tutor") {
            return res.status(403).json({
                success: false,
                message: "This route is only for admins or tutors.",
            });
        }
        next();
    } catch (error) {
        console.error("Error in isAdminOrTutor middleware:", error.message);
        return res.status(500).json({
            success: false,
            message: "Error verifying user role.",
        });
